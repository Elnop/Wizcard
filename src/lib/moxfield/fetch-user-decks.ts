import type { MoxfieldUserDeckEntry } from '@/app/api/moxfield/user/decks/route';
...
export async function fetchMoxfieldUserDecks(username: string): Promise<MoxfieldUserDeckEntry[]> {
	const res = await fetch(`/api/moxfield/user/decks?userName=${encodeURIComponent(username)}`, {
		credentials: 'include',
	});

	if (res.status === 403)
		throw new Error('Access denied — make sure you are logged in to Moxfield.');
